...
<div style="padding:60px 0;">
    <div class="container">
        <div style="max-width:900px; margin:auto; background:#fff; border-radius:24px; padding:30px; box-shadow:0 20px 45px rgba(15,23,42,0.08);">
            <h1 style="font-size:32px; margin-bottom:8px;">Edit Package</h1>
            <p style="color:#64748b; margin-bottom:24px;">Update package/plan information.</p>
            ...
            <form action="{{ route('admin.packages.update', $package->id) }}" method="POST">
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:18px;">
                    <div style="margin-bottom:18px;">
                        ...
                        <input type="number" step="0.01" min="0" name="price" value="{{ old('price', $package->price) }}"
                            style="width:100%; padding:14px 16px; border:1px solid #cbd5e1; border-radius:14px;">
                        @error('price')
                            <div style="color:#dc2626; font-size:13px; margin-top:6px;">{{ $message }}</div>
                        @enderror
                ...
                <div style="margin-bottom:22px;">
                    ...
                    <textarea name="features" rows="9"
                        style="width:100%; padding:14px 16px; border:1px solid #cbd5e1; border-radius:14px; resize:vertical;">{{ old('features', $package->features) }}</textarea>
                ...
                <label style="display:flex; align-items:center; gap:10px; margin-bottom:24px; color:#334155; font-weight:700;">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $package->is_active) ? 'checked' : '' }} style="width:18px; height:18px; accent-color:#2563eb;">
